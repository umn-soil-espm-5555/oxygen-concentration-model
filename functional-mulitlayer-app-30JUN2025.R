              ...
              fluidRow(
                column(3,
                  ...
                  conditionalPanel(
                    ...
                    numericInput("layer1_water", "Water Content", value = 0.25, min = 0.05, max = 0.6, step = 0.01),
                ...
                column(3,
                  ...
                  conditionalPanel(
                    ...
                    numericInput("layer2_water", "Water Content", value = 0.30, min = 0.05, max = 0.6, step = 0.01),
                ...
                column(3,
                  ...
                  conditionalPanel(
                    ...
                    numericInput("layer3_water", "Water Content", value = 0.35, min = 0.05, max = 0.6, step = 0.01),
                ...
                column(3,
                  ...
                  conditionalPanel(
                    ...
                    numericInput("layer4_water", "Water Content", value = 0.32, min = 0.05, max = 0.6, step = 0.01),
...
server <- function(input, output, session) {
  ...
  get_active_layers <- reactive({
    ...
    if (input$layer1_active) {
      layers[[1]] <- list(
        ...
        porosity = input$layer1_porosity, water = input$layer1_water,
    ...
    if (input$layer2_active) {
      layers[[length(layers) + 1]] <- list(
        ...
        porosity = input$layer2_porosity, water = input$layer2_water,
    ...
    if (input$layer3_active) {
      layers[[length(layers) + 1]] <- list(
        ...
        porosity = input$layer3_porosity, water = input$layer3_water,
    ...
    if (input$layer4_active) {
      layers[[length(layers) + 1]] <- list(
        ...
        porosity = input$layer4_porosity, water = input$layer4_water,
  ...
  calculate_layer_properties <- reactive({
    ...
    # Assign properties based on layers
  ...
  calculate_diffusion <- reactive({
    ...
    # More realistic tortuosity calculation that's very sensitive to air content
    if (input$tortuosity_model == "millington_quirk") {
      # Classic Millington-Quirk model - very sensitive to air content
      tortuosity <- (air_porosity^(10/3)) / (layer_props$porosity^2)
    } else if (input$tortuosity_model == "moldrup") {
      # Moldrup model - also quite sensitive
      tortuosity <- (air_porosity^2.5) / (layer_props$porosity^1.5)
    } else {
      tortuosity <- rep(input$custom_tortuosity, length(layer_props$depths))
    }
    
    # Ensure minimum tortuosity but make it very small for waterlogged conditions
    tortuosity[tortuosity < 1e-6] <- 1e-6
    
    # Effective diffusion coefficient - HIGHLY sensitive to air content
    # Use the relative diffusivity approach: D_eff/D0 = (θ_a/φ)^n * τ
    # where n is typically 3-4 for very strong dependence on air content
    air_fraction <- air_porosity / layer_props$porosity
    
    # Make diffusion EXTREMELY sensitive to air content
    # When air fraction drops below ~0.1, diffusion becomes negligible
    sensitivity_factor <- air_fraction^3.5  # Very strong nonlinear dependence
    
    D_eff <- D0 * air_fraction * tortuosity * sensitivity_factor
    
    # For very low air content (waterlogged), diffusion is practically zero
    waterlogged_threshold <- 0.05  # If air porosity < 5% of total porosity
    waterlogged_mask <- air_fraction < waterlogged_threshold
    D_eff[waterlogged_mask] <- D_eff[waterlogged_mask] * 0.001  # Reduce by 1000x
  ...
  # Main model calculation using numerical approach for multi-layer
  observeEvent(input$update_model, {
    ...
    cat("Respiration range:", range(total_respiration), "\n")
    
    # Use analytical solution for now to ensure correct physics
    # This is the classic steady-state diffusion-respiration equation
    n <- length(depths)
    concentration <- numeric(n)
    
    # For each depth, solve approximately using exponential decay
    # but with layer-specific properties
    for (i in 1:n) {
      depth <- depths[i]
      
      # Find average properties from surface to this depth
      if (i == 1) {
        avg_D <- D_eff[1]
        avg_R <- total_respiration[1]
      } else {
        avg_D <- mean(D_eff[1:i])
        avg_R <- mean(total_respiration[1:i])
      }
      
      # Analytical solution: C(z) = C_a * exp(-z * sqrt(R/D))
      if (avg_D > 0 && avg_R >= 0) {
        penetration_depth <- sqrt(avg_D / max(avg_R, 1e-12))
        concentration[i] <- C_a * exp(-depth / penetration_depth)
      } else {
        concentration[i] <- C_a * exp(-depth * 1e3)  # Very rapid decay if D is tiny
      }
    }
    
    # Ensure non-negative and reasonable concentrations
    concentration <- pmax(0, pmin(concentration, C_a))
  ...
  output$calculated_params <- renderText({
    ...
    # Calculate air content metrics
    surface_air_fraction <- diffusion_data$air_fraction[1]
    min_air_fraction <- min(diffusion_data$air_fraction)
    ...
    paste(
      ...
      "",
      paste("Air Content Analysis:"),
      paste("  Surface air fraction:", round(surface_air_fraction, 3)),
      paste("  Minimum air fraction:", round(min_air_fraction, 3)),
      paste("  Waterlogged layers:", sum(diffusion_data$air_fraction < 0.05)),
      "",
