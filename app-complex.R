...
server <- function(input, output, session) {
  ...
  calculateProfile <- reactive({
    ...
    for(i in 1:nrow(layers)) {
      ...
      if(i == 1) {
        ...
        # Calculate C at top of current layer based on continuity
        # First calculate C at bottom of previous layer
        prev_layer <- layers[i-1, ]
        x_bottom_prev <- layers$H_lower[i-1]
        
        # Sum of respiratory activities from layers above previous layer
        if(i > 2) {
          sum_qa_above_prev <- sum(layers$q[1:(i-2)] * layers$B[1:(i-2)])
        } else {
          sum_qa_above_prev <- 0
        }
        
        # C at bottom of previous layer (which equals C at top of current layer)
        C_top <- input$C0 - (2 * sum_qa_above_prev * (x_bottom_prev - layers$H_upper[i-1]) - 
                            prev_layer$q * (x_bottom_prev - layers$H_upper[i-1])^2) / (2 * prev_layer$D)
      }
    ...
    # Remove any negative concentrations (set to 0)
    results$C[results$C < 0] <- 0
  ...
  output$summaryTable <- renderTable({
    layers <- data.frame(
      ...
      Thickness = c(input$B1, input$B2, input$B3, input$B4),
      q = c(input$q1, input$q2, input$q3, input$q4),
      D = c(input$D1, input$D2, input$D3, input$D4)
    ) %>%
      filter(Thickness > 0)
    
    # Set column names for display
    names(layers) <- c("Layer", "Thickness (cm)", "q (mg/m³s)", "D (×10⁻⁷ m²/s)")
